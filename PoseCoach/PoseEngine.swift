...
final class PoseEngine: NSObject, ObservableObject {
    ...
    private func setupLandmarker() {
        ...
        if let heavy = Bundle.main.path(forResource: "pose_landmarker_heavy", ofType: "task") {
        options.baseOptions.modelAssetPath = heavy
        modelUsed = "pose_landmarker_heavy"
        } else if let lite = Bundle.main.path(forResource: "pose_landmarker_lite", ofType: "task") {
        options.baseOptions.modelAssetPath = lite
        modelUsed = "pose_landmarker_lite"
        } else {
        print("⚠️ No pose model found. Add pose_landmarker_heavy.task or pose_landmarker_lite.task to the target.")
        }
    ...
    private func process(buffer: CMSampleBuffer) {
        guard let pixelBuffer = CMSampleBufferGetImageBuffer(buffer), let landmarker = landmarker else { return }
        ...
        do {
            result = try landmarker.detect(videoFrame: videoFrame, timestampInMilliseconds: timeMs)
        } catch {
        ...
        DispatchQueue.main.async {
            ...
            self.landmarks = pose.map { Landmark(x: CGFloat($0.x), y: CGFloat($0.y), z: CGFloat($0.z ?? 0), c: CGFloat($0.visibility ?? 0)) }
    ...
    private func updateAnglesIfPossible() {
        ...
        guard landmarks.count > max(PoseSpec.leftAnkle, PoseSpec.leftShoulder) else { return }
        ...
        let anklePt = CGPoint(x: landmarks[ankleIdx].x, y: landmarks[ankleIdx].y)
        let shoulderPt = CGPoint(x: landmarks[shoulderIdx].x, y: landmarks[shoulderIdx].y)
